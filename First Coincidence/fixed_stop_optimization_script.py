...
trial, gain, and loss metrics leaving
...
if __name__ == "__main__":
    ...
    for train_index, test_index in tqdm(logo.split(X, y, runs_ids)):
        ...
        for cv_train_index, cv_val_index in logo.split(
            ...
            # Step 7: Average along all the runs
            bar_t.append(t.mean() + 1)  # +1 because it is an index

        # Steps 8 & 9: Average along all the folds
        bar_bar_t = np.round(np.mean(bar_t))
        bar_bar_t_vals.append(bar_bar_t)

        # Step 10: Obtaining GAIN & LOSS metrics on test
        # Reshaping from
        # · X: (n_runs, n_epochs, n_feats) -> (n_runs x n_epochs, n_feats)
        # · y: (n_runs, n_epochs) -> (n_runs x n_epochs)
        ...
        score_cur = scores_test[int(bar_bar_t) - 1]
        ...
        # Extra values
        t = int(bar_bar_t) - 1  # -1 because it is not an index!
        es_extra_vals(
        ...
        # Save results
        es_save_session_vals(
            results, session_idx, bar_bar_t,
            bar_bar_t, gain_curr, loss_curr,
            score_ref, score_cur
        )
    ...
    # Steps 11 & 12: Average along all the sessions left-out.
    bar_bar_bar_t = np.mean(bar_bar_t_vals)
    es_save_vals(
        results,
        "Fixed Stop - First Coincidence",
        "trial", bar_bar_bar_t,
        bar_bar_bar_t, gain_vals, loss_vals
    )
