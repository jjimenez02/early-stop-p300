...
the gain & loss metrics (taking the intersection
of both curves) and performing the aforementioned
test just over the predicted stimuli by Soft-Voting.
...
if __name__ == "__main__":
    ...
    for train_index, test_index in tqdm(logo.split(X, y, runs_ids)):
        ...
        for fold_idx, (cv_train_index, cv_val_index) in enumerate(logo.split(
            ...
            for a_idx, a in enumerate(ALPHAS):
                # Obtain trial and prediction per run
                t, z = stat_test_all_stim_es_trials(
                    p_vals_pred_cv, y_pred_cv,
                    a, args.bonferroni
                )
        ...
        # Obtain trial and prediction per run
        t, z_t = stat_test_all_stim_es_trials(
            p_vals_pred, y_pred,
    ...
    es_save_vals(
        results,
        f"Stat Test - G-L Bal - {args.stat} - Bonferroni: {args.bonferroni}",
