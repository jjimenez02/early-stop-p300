...
3. Preprocess the data and save the results within the `preproc/` folder by executing (this may take a minute):
```shell
...
```shell
$ python scripts/Plots/prepare_data.py -id . -o out/all_results.pkl
```
6. Generate the **local group comparisons**. This will generate a `Results` folder automatically, it may take some seconds as they are quite a few:
```shell
$ ./scripts/Plots/plot_all.sh out/all_results.pkl
```
7. Generate the **global comparisons** by executing:
```shell
$ ./scripts/Plots/plot_final_comparison.sh out/all_results.pkl Results/Global_Comparisons
```
8. Generate the plots responsible for comparing the three best **local group methods in a single plot** (Figure 4.1.4 from the thesis):
```shell
$ ./scripts/Plots/plot_local_group_comparison.sh out/all_results.pkl Results/Local_Group
```
9. Generate the Area Under the Curve bar-plots for all the Gain-Conservation optimised Statistical Tests:
```shell
$ ./scripts/Plots/plot_stat_tests_comparison.sh out/all_results.pkl Results/Stat_Tests
```
10. Generate the Appendix figures:
```shell
$ ./scripts/Plots/plot_appendix_local_group_comparison.sh out/all_results.pkl Results/Appendix/Local_Group

$ ./scripts/Plots/plot_appendix_stat_tests_comparison.sh out/all_results.pkl Results/Appendix/Stat_Tests
```
